...
class SupabaseProductTools:
    ...
    async def search_by_query(self, user_query: str, match_count: int = 5) -> List[Dict[str, Any]]:
        # Sanitize input
        sanitized_query = sanitize_search_query(user_query)
        if not sanitized_query:
            return []

        validated_count = validate_match_count(match_count)
        embedding = await self._embed(sanitized_query)

        def _call() -> Any:
            return self.client.rpc(
                self.match_rpc,
                {"query_embedding": embedding, "match_count": validated_count},
            ).execute()
    ...
    async def get_by_id(self, product_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Invalid product_id: %s - %s", product_id, e)
            return []
    ...
    async def get_by_photo_url(self, photo_url: str, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            validated_url = validate_url(photo_url, allow_any_domain=True)
        except ValidationError as e:
            logger.warning("Invalid photo_url: %s - %s", photo_url, e)
            return []

        # Escape special LIKE characters to prevent pattern injection
        escaped_url = escape_like_pattern(validated_url)
        validated_limit = validate_match_count(limit, min_val=1, max_val=10)

        def _call() -> Any:
            return (
                self.client.table(self.products_table)
                .select("*")
                .ilike("colors::text", f"%{escaped_url}%")
