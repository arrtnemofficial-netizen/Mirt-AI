...
from src.core.constants import AgentState as StateEnum, EscalationLevel, MessageRole
...
class ConversationState(TypedDict):
    """TypedDict for LangGraph state. Use StateEnum for state values."""
    messages: List[Dict[str, Any]]
    current_state: str
    metadata: Dict[str, Any]


async def agent_node(state: ConversationState, runner) -> ConversationState:
    """Single LangGraph node that calls the agent and updates state."""

    state.setdefault("metadata", {})
    state.setdefault("messages", [])
    state.setdefault("current_state", StateEnum.default())

    state["metadata"]["current_state"] = state.get("current_state", StateEnum.default())
    ...
    prepared_metadata = apply_metadata_defaults(state.get("metadata"), state.get("current_state", StateEnum.default()))
