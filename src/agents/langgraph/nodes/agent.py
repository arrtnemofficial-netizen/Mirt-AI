...
async def agent_node(
    ...
    # =========================================================================
    snippet_result = check_snippet_policy(state, user_text, detected_intent)
    if snippet_result:
    ...
    # =========================================================================
    color_result = handle_color_request(state, user_text)
    if color_result:
        return color_result
    ...
    # Context Trimming
    trimmed_messages = trim_message_history(state.get("messages", []))
    ...
    try:
        ...
        updated_products = merge_cart(
            current_products=state.get("selected_products", []) or [],
        ...
        if response.customer_data:
            ...
            if c_data.name: metadata_update["customer_name"] = c_data.name
            if c_data.phone: metadata_update["customer_phone"] = c_data.phone
            if c_data.city: metadata_update["customer_city"] = c_data.city
            if c_data.nova_poshta: metadata_update["customer_nova_poshta"] = c_data.nova_poshta
