...
def detect_color_show_request(text: str) -> bool:
    ...
    show_patterns = [
        ...
        r"показати.*кольор",
        r"показати.*інш",
        r"це.*не.*той.*кольор",  # "це не той колір"
        r"не.*той.*кольор",  # "не той колір"
        r"інший.*кольор",  # "інший колір"
        r"другий.*кольор",  # "другий колір"
    ]
...
def get_product_name_for_color_show(state: dict[str, Any]) -> str | None:
    ...
    if selected and isinstance(selected, list) and len(selected) > 0:
        ...
        if isinstance(first_product, dict):
            ...
            if product_name:
                # Remove color from name if present (e.g., "Сукня Анна (червона)" -> "Сукня Анна")
                name_str = str(product_name).strip()
                # Remove color in parentheses
    ...
    if isinstance(metadata, dict):
        ...
        if upsell_name:
            name_str = str(upsell_name).strip()
            name_str = re.sub(r'\s*\([^)]*\)\s*$', '', name_str)
            return name_str
