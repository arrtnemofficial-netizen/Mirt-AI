...
from src.core.state_machine import Intent, State, get_next_state, normalize_state
...
async def tool_plan_node(state: ConversationStateV2) -> ConversationStateV2:
    ...
    if plan.has_tools:
        ...
        # Log tool execution
        sum(len(tr.get("result") or []) for tr in result.get("tool_results", []))
...
async def validation_node(state: ConversationStateV2) -> ConversationStateV2:
    ...
    if products:
        _valid_products, product_errors = ProductAdapter.batch_validate(products)
...
def _get_latest_user_content(messages: list[dict[str, Any]]) -> str | None:
...
def _build_moderation_escalation(
    metadata: dict[str, Any], current_state: str, reason: str
) -> AgentResponse:
...
_compiled_graph_v2: CompiledGraph | None = None
...
def get_active_graph():
    ...
    Always returns v2 graph (5-node architecture).
    V1 is deprecated and removed.
    """
