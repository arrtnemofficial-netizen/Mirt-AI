...
> **Останнє оновлення:** 7 грудня 2025  
> **Версія:** 2.0 (після оптимізації)  
> **Статус:** ✅ 646 тестів пройдено, 3 warnings (сторонні бібліотеки)
...
│                                                              │
│  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐  │
│  │ Moderat. │ → │  Intent  │ → │   Plan   │ → │  Agent   │  │
│  └──────────┘   └──────────┘   └──────────┘   └──────────┘  │
│                                                              │
│  Ноди: vision_node, support_node, payment_node, etc.         │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│   │   │   ├── models.py         # Pydantic моделі відповідей
│   │   │   └── deps.py           # AgentDeps (залежності)
│   │   │
│   │   └── langgraph/            # LangGraph оркестрація
│   │       ├── graph_v2.py       # Головний граф
│   │       └── nodes/            # Ноди графа
...
│   │   ├── order_model.py        # Модель замовлення
