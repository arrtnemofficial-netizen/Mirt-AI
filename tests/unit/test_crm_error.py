...
class TestCrmErrorNode:
    ...
    async def test_first_entry_analyzes_error(self):
        ...
        mock_handler.handle_crm_failure = AsyncMock(return_value={
            "strategy": "retry",
            "can_retry": True,
        ...
        with patch(
            ...
            with patch("src.agents.langgraph.nodes.crm_error.log_agent_step"):
...
class TestHandleEscalateChoice:
    ...
    async def test_escalation_goes_to_end(self):
        ...
        mock_handler = MagicMock()
        mock_handler.escalate_to_operator = AsyncMock(return_value={
