...
SNIPPET_CASES = _build_100_cases(
    _parse_snippets(PromptRegistry().get("system.snippets").content)
)
SNIPPET_CASE_IDS = [
    f"{title}::{rule_id}::{idx}"
    for idx, (title, _body, rule_id) in enumerate(SNIPPET_CASES)
]
...
@pytest.mark.parametrize("title,body,rule_id", SNIPPET_CASES, ids=SNIPPET_CASE_IDS)
def test_snippets_cases(title: str, body: str, rule_id: str) -> None:
    _assert_rule(title=title, body=body, rule_id=rule_id)
