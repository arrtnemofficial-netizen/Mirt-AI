...
from src.types import (
    ModelsConfig, TestSuite, ModelConfig, TestCase
)
...
async def call_assistant_model(
    ...
    if not api_key:
        return {
            "error": f"API key not found: {model.api.api_key_env}",
            "raw_response": None
        }
...
def rule_based_judge(test: TestCase, assistant_response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if hasattr(test.expected.rules, 'must_have_products') and test.expected.rules.must_have_products:
...
async def run_prompt_tests(dataset_name: str = "golden_mirt_v1.json"):
    ...
    print(f"\n{'='*60}")
    print(f"🧪 MIRT AI Prompt Evaluation")
    print(f"{'='*60}")
    ...
    for i, test in enumerate(test_suite.tests, 1):
        for model in assistants:
            print(f"[{i}/{len(test_suite.tests)}] {test.id}: {test.description[:40]}... ", end="", flush=True)
