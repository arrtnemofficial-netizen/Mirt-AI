...
def _run_supabase_connection():
    """Test 1: Supabase connection."""
    print_header("TEST 1: Supabase Connection")

    client = get_supabase_client()
    if not client:
        print_result("Connection", True, "No client returned (Supabase not configured)")
        return True
...
def main():
    ...
    # Run all tests
    results.append(("Supabase Connection", _run_supabase_connection()))
    results.append(("Follow-up Logic (1-min)", _run_followup_logic_1min()))
    results.append(("Summarization Logic", _run_summarization_logic()))
    results.append(("Real MessageStore", _run_message_store_real()))
    results.append(("Dispatcher Sync", _run_dispatcher_sync()))
    results.append(("Full Follow-up Flow", _run_full_followup_flow()))
...
def test_supabase_connection():
    assert _run_supabase_connection()


def test_followup_logic_1min():
    assert _run_followup_logic_1min()


def test_summarization_logic():
    assert _run_summarization_logic()


def test_message_store_real():
    assert _run_message_store_real()


def test_dispatcher_sync():
    assert _run_dispatcher_sync()


def test_full_followup_flow():
    assert _run_full_followup_flow()
