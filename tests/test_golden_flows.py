...
class TestTextDiscoveryFlow:
    ...
    def test_discovery_flow_routing(self):
        ...
        intent = detect_intent_from_text(
            "Які є костюми для дівчинки?", has_image=False, current_state="STATE_0_INIT"
        )
        assert intent in ["DISCOVERY_OR_QUESTION", "PRODUCT_CATEGORY"]

        # Step 2: Size question in DISCOVERY
        intent = detect_intent_from_text(
            "Зріст 140", has_image=False, current_state="STATE_1_DISCOVERY"
        )
        assert intent in ["SIZE_HELP", "DISCOVERY_OR_QUESTION"]

        # Step 3: In OFFER, confirmation
        intent = detect_intent_from_text(
            "так, підходить", has_image=False, current_state="STATE_4_OFFER"
        )
        assert intent == "PAYMENT_DELIVERY"
...
class TestRejectionFlow:
    ...
    def test_rejection_in_offer(self):
        ...
        intent = detect_intent_from_text(
            "дякую, подумаю", has_image=False, current_state="STATE_4_OFFER"
        )
...
class TestComplaintFlow:
    ...
    def test_complaint_always_escalates(self, complaint_text):
        ...
        for state in [
            ...
            State.STATE_5_PAYMENT_DELIVERY.value,
        ]:
            # Detect intent
...
class TestPaymentDataCollection:
    ...
    def test_payment_inputs_stay_in_payment(self, payment_input):
        ...
        intent = detect_intent_from_text(
            payment_input, has_image=False, current_state="STATE_5_PAYMENT_DELIVERY"
        )
        assert intent == "PAYMENT_DELIVERY", (
            f"'{payment_input}' should be PAYMENT_DELIVERY, got {intent}"
        )
...
class TestProductSelectionInOffer:
    ...
    # - Сукня Анна (голубий, малина, чорний, червоний, шоколадний, рожевий, сірий)
    # - Костюм Валері (універсальний)
    # - Костюм Ритм (рожевий, шоколадний, бордовий)
    # - Костюм Каприз (рожевий, бордовий, шоколадний)
    ...
    def test_product_selection_triggers_payment(self, product_name, expected):
        ...
        intent = detect_intent_from_text(
            product_name, has_image=False, current_state="STATE_4_OFFER"
        )
...
class TestEdgeCases:
    ...
    def test_special_characters(self):
        ...
        intent = detect_intent_from_text(
            special_text, has_image=False, current_state="STATE_0_INIT"
        )
        assert intent is not None
