...
- **Supabase тулзи без «вайбу»**: ретраї, явний `SupabaseToolError`, валідація каталогу (id/ціна/фото) і детермінований фолбек ембеддингів (`src/services/supabase_tools.py`).
...
- **Персистенція сесій**: додано опційний Supabase store (jsonb state у наявній таблиці); для high-load ще потрібні кеш/TTL та ретрай логіка (для каталогу вже покрито).
