...
2. Import smoke: `src.agents.pydantic.shared.model_factory` (Pydantic AI)
3. Import smoke: `src.agents.langgraph.state.create_initial_state` (LangGraph)
4. `pytest -q tests/unit/test_prompt_compliance.py`
5. `pytest -q tests/unit/test_prompt_contract_snapshot.py`
6. `pytest -q tests/test_vision_contract.py`
...
4. Прямих конфліктів версій `pydantic-ai` ↔ `langgraph` у маніфестах не виявлено; блокер локальної перевірки — Python 3.10 (зокрема `datetime.UTC`, що потребує 3.11+).
