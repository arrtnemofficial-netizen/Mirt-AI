...
laguna = client.table('products').select('id, name, description').like('name', 'Костюм Лагуна%').execute()
for p in laguna.data:
    new_desc = f"{p.get('description', '')} [ЗАСТІБКА: ПОВНА блискавка від горла до низу. МАТЕРІАЛ: плюш]"
    client.table('products').update({'description': new_desc}).eq('id', p['id']).execute()
    print(f"✅ {p['name']}: full_zip")

# Get all Мрія products
mriya = client.table('products').select('id, name, description').like('name', 'Костюм Мрія%').execute()
for p in mriya.data:
...
print("\nВерифікація:")
verify = client.table('products').select('name, description').or_('name.like.Костюм Лагуна%,name.like.Костюм Мрія%').execute()
