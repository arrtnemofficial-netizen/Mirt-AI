...
def main():
    ...
    try:
        client = create_client(settings.SUPABASE_URL, settings.SUPABASE_API_KEY.get_secret_value())
        print("   ✅ Підключено!")
...
""")
    ...
    result = (
        client.table("products")
        .select("name, price_by_size")
        .not_.is_("price_by_size", "null")
        .execute()
    )
